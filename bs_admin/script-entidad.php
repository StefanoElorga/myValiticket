...
*/
		...
		$folder_u="../multimedia/imagenes/cs_entidades/";

		$imag=$_FILES['photoupload'];

		$ext=extension($imag['name']);
		if ($ext=="jpg" or $ext=="jpeg") {
			$newname=$insert."-".date("YmdHis")."-".rand(10000,99999).".jpg";
			}
		if ($ext=="png") {
			$newname=$insert."-".date("YmdHis")."-".rand(10000,99999).".png";
			}
		...
		if ($height>$width) {
			...
			redimensionar($ext,$imag['tmp_name'],$folder_u.$newname."_big.jpg",1280,95,'');

			redimensionar($ext,$imag['tmp_name'],$folder_u.$newname,644,95,'');
		...
		ginsert("INSERT INTO cs_entidades_fotos (nombre, ida) VALUES ('$newname', $insert)");
